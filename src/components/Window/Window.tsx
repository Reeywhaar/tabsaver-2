...
export const Window: FunctionComponent<{ window: WindowDescriptor; index: number }> = ({ window, index }) => {
  ...
  const discardHandler = useClickHandler(async () => {
    const activeTab =
      windowTabs.find(
        t => t.active && (t.url === 'about:newtab' || t.url === 'about:home' || t.url === 'about:blank' || t.url === 'chrome://browser/content/blanktab.html')
      ) ??
      windowTabs.find(t => t.url === 'about:newtab' || t.url === 'about:home' || t.url === 'about:blank' || t.url === 'chrome://browser/content/blanktab.html')
    if (activeTab) {
